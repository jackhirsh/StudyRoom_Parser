...
calendar_text = open("/home/hirshjack/git/StudyRoom_Parser/Calendar.txt", "r")
...
for i in range(1, len(month)):
    ...
    print("Room #" + str(room_num[i]) + " - From: " +
          str(cur_start) + " To " + str(cur_end))

file_out = "Calendar_Events.csv"

with open(file_out, 'w') as csv_file:
    fieldnames = ['Subject', 'Start Date',
                  'Start Time', 'End Date', 'End Time', 'Location']
    ...
    for i in range(0, len(month)):
        ...
        writer.writerow({'Subject': "Study Room", 'Start Date': start_date,
                         'Start Time': start, 'End Date': end_date,
                         'End Time': end, 'Location': location})
